...
\defin{(Ultra-)Filter}{Suppose $\mathcal{B}\models BA$. A non-empty subset $F\subseteq B$ is called a filter of $\mathcal{B}$, if
\begin{itemize}
    \item $0\notin F$
    \item $\forall a \forall b a\in F \land b\in F \to a\cdot b\in F$
    \item $\forall a \forall b a\in F \land a \leq b \to b\in F$
\end{itemize}
An \imp{Ultrafilter}of $\mathcal{B}$ is a filter $\mathcal{F}$ such that $\forall a\in B a\in \mathcal{F} \lor \overline{a}\in \mathcal{F}$.\\

}
\noindent We define $S(B)$,\outernote{$S(B)$}the stonespace of $\mathcal{B}$ to be 
$S(B) \defeq\{\text{the set of all ultrafilters of $\mathcal{B}$} \}$

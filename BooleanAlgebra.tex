...
\defin{Stone space}{
    ...
    and call it the Stone space of $\mathcal{B}$.\outernote{$S(\mathcal{B})$}
}
...
\note{}{
    ...
    \begin{enumerate}
        ...
        \item $\langle a\rangle \defeq \{x\in S(\mathcal{B}): a\in x\}$ where $a$ runs through $B$ forms a basis for a topology on the Stone space of $\mathcal{B}$
    \end{enumerate}
}

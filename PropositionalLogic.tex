...
\defin{Satisfaction}{A TA $\nu$ satisfies\outernote{satisfy} a prop. sent. $\alpha$ 
    ...
    We call $\alpha$ satisfiable\outernote{satisfiable} iff there exists a TA that satisfies it.}
...
\bsp{}{
    \begin{enumerate}
        \item Let $\mathcal{U}$ be the set of all expressions, $\mathcal{B}$ the set of atoms and $\mathcal{F}=\{\varepsilon_\square:  \square in \{\lnot, \land,\lor,\to,\leftrightarrow \}\}$

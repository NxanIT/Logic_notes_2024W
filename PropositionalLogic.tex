...
\thm{(Post)}{\footnote{Emil Post}
    Let $G$ be an $n$-ary boolean function for $n\geq 1$.\outernote{$n$-ary boolean function}Then there is a prop. sent. $\alpha$ such that. $B_\alpha = G$.
...
\coroll{$\{\lnot,\land,\lor\}$ is a complete\outernote{completeness (connectives)}set of logical connectives, i.e. every prop. sent. is tautologically 

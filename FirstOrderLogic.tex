...
\bsp{}{
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item [$E$] is a binary function symbol (exponentiation) %TODO or perhaps unary
...
\defin{Term Building Operations}{
    ...
    \imp{$\TERMS$}, the set of all terms is defined as the set of expressions that are built up from variables and constants by applying the term building operations
    finitely many times.
}
...
\defin{Free variables}{
    ...
    \begin{enumerate}
        ...
        \item If $\varphi \equiv \forall v_i \alpha$ then $x$ occurs free in $\varphi$ iff $x$ occurs free in $\alpha$ and $x\neq v_i$
    \end{enumerate}
...
The equivalent scheme to our TA in predicate logic. The meaning of formulas is given by {structures}, 
which also determine the scope of the quantifier $\forall$, the meaning of all parameters.
\defin{structure}{A \imp{structure} $\mathcal{A}$ for a first order language $\mathcal{L}$ is a non-empty set $A$ called \graybf{universe} or \graybf{underlying set of $\mathcal{A}$} together with an interpretation of each parameters of $\mathcal{L}$ i.e.
    \begin{itemize}
...
\defin{Satisfy}{ We define '$\mathcal{A}$ satisfies $\varphi$ with $s$' and write
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \begin{itemize}
            ...
            \item $\mathcal{A}\models \forall x \varphi \: [s]$ iff for all $a\in A$ we have $\mathcal{A} \models \varphi [s(x| a)]$, where 
            \begin{equation*}
...
\bsp{}{
    Let $\mathcal{L} = \{\forall, \leq, S, 0\}$ be our language.
    An example of an $\mathcal{L}$-structure is $\mathcal{N} = (\NN,\leq^\mathcal{N},S^\mathcal{N},0^\mathcal{N})$. Together with a specified assignment
    $s: v_n \mapsto n-1$ we can analyse 
    \begin{itemize}
        \item $s(v_1) = 0$
...
To know wheter $\mathcal{A}\models \varphi \: [s]$ it suffices to know where $s$ maps the variables that occur free in $\varphi$. This results from the Coincidence \ref{Thm.evalonlyfreevar}
\thm{Coincidence Lemma}{\label{Thm.evalonlyfreevar}Suppose $s_1,s_2:V\to A$ agree on all variables that occur free in $\varphi$ then 
 $$\mathcal{A}\models \varphi \:[s_1] \text{ iff }\mathcal{A}\models \varphi \: [s_2]$$
    ...
    \begin{enumerate}
        ...
        \item \datenote{22.10.2024}
        if $\varphi$ is $\lnot \alpha$ then $\mathcal{A}\models \lnot \alpha  \: [s_1]$ iff $\mathcal{A}\not\models \alpha  \: [s_1]$ iff $\mathcal{A}\not\models \alpha  \: [s_2]$ iff $\mathcal{A}\models \lnot \alpha  \: [s_1]$
...
\defin{Logical implication}{
    ...
    for every $\mathcal{L}$-structure $\mathcal{A}$ and for every assigment $s:V\to A$ 
...
\begin{enumerate}
    ...
    We need a counterexample to $ P x_2 \models \forall x_1 P x_1$. Let $A = \{a_1,a_2\}$ $s(x_2) = a_1$ and $P^\mathcal{A} = \{a_1\}$ %TODO skizze einfügen
...
\defin{definability in a structure}{ 
    ...
    \[P = \bigl\{(a_1,\dots a_n) \in A^n: \mathcal{A} \models \varphi\: \llbracket a_1,\dots a_n\rrbracket\bigr\}\]
    ...
    \[P = \bigl\{(a_1,\dots a_n) \in A^n: \mathcal{A} \models \varphi\: \llbracket a_1,\dots a_n, b_1,\dots b_m\rrbracket\bigr\}\]
}
\bsp{}{
    \begin{enumerate}
...
\note{}{Intuatively a Homomorphism of $\mathcal{A}$ into $\mathcal{B}$ is a map $A \to B$ that preserve all funciton and relation symbols in some sense, (in general however not the definable relations)}
\defin{Isomorphism}{
    \begin{itemize}
        \item $h:A\to B$ is called isomorphism of $\mathcal{A}$ \imp{into} $\mathcal{B}$ if $h$ is an injective homomorphism $A\to B$ \\
        (in other textbooks it may be called an isomorphic embedding of $\mathcal{A}$ into $\mathcal{B}$)
        \item $h:A\to B$ is called isomorphism of $\mathcal{A}$ \imp{onto} $\mathcal{B}$, if \\
        $h$ is a bijective homomorphism $A\to B$
        \item $\mathcal{A}$\outernote{isomorphic}and $\mathcal{B}$ are called isomorphic if there exists an isomorphism $h:A\to B$ of $\mathcal{A}$ onto $\mathcal{B}$
    \end{itemize}
}
\note{}{
    %TODO: in general 
...
\newpage
\thm{Homomorphism Theorem}{\datenote{29.10.2024}
...
\note{}{$\mathcal{A}\cong \mathcal{B}$ implies $\mathcal{A}\equiv \mathcal{B}$
    ...
    For instance DLO (dense linear order) w/o endpoints is complete, so two structures on DLO are equivalent 
...
\note{}{\ref{HomThm:coroll} can be used to show that some $R\subseteq A^n$ is not definable in $\mathcal{A}$
}
\bsp{}{
...
\section{Unique readability for terms and formulas}
\subsection*{Unique readability for terms}
\defin{}{
    We define a function $K: \TERMSYMB\to \ZZ$ on the set $\TERMSYMB$ of symbols from which terms are built up (variables, constants, function symbols).
    $$\TERMSYMB = \VARIAB \cup \CONST \cup \FUNCT$$
    $K(s) \defeq 1-n$ where $s\in \TERMSYMB$ and $n$ is the number of terms that need to follow s in order to obtain a term.
    More precicely
    \begin{enumerate}[label=(\roman*)]
        \item $K(x) \defeq 1$ for $x\in \VARIAB$ 
        \item $ K(c)\defeq 1$ for $c\in \CONST$
        \item and $K(f) = 1-n$ for $f\in \FUNCT$, where $n$ is the arity of the function symbol $f$.
    \end{enumerate}
    
    $K$ extends to a function on the set $\overline{\TERMSYMB}$ of all expressions build up from the set $\TERMSYMB$ by setting
    $$K(s_1,\dots s_n) = K(s_1) + \dots + K(s_n)$$ 
}
The idea is that $K$ tells us the number of symbols that at least need to follow to obtain a formula. (This number is unique because no symbol is a finite sequence of other symbols)
Note that the following set inclusions hold:
$\TERMS\subseteq \overline{\TERMSYMB}\subseteq \EXPR$.
\lemma{}{
    Let $t\in \TERMS$ be a term. Then $K(t)=1$
}{
...
\prop{Unique readability for terms}{
    ...
    \item $\mathcal{F}_f|_{\text{terms}}$ are one-to-one (injective):
        ...
        then one is a proper initial segment of the other. By \ref{FO:Unique-readability-Ko} 
        its $K$-value has to be less than $1$ so it is not a term. By induction we get 
        $t_1 = t_1', \dots, t_n=t_n'$.
    \end{itemize}
}
\subsection*{Unique readability for formulas}
\lemma{}{$K:\overline{\TERMSYMB}\to \ZZ$ extends to a function $K:\EXPR\to \ZZ$ on the set of all expressions by adding to the above definition:
    \begin{enumerate}[label=(\roman*)]
        \setcounter{enumi}{3}
        \item $K(() \defeq -1$ and $K()) \defeq 1$ 
        \item $K(\forall) \defeq 1$, $K(\lnot) \defeq 0$ and $K(\to) \defeq -1$ 
        \item $K(P) \defeq 1-n$ for an $n$-ary rel. symb. $P$. 
        \item $K(=) \defeq -1$
        \item $K(s_1\dots s_n) \defeq K(s_1) + K(s_n)$ for $s_i\in \TERMS\cup\{(,),\forall,\lnot,\to,P,\dots,=\}$
        \item $K(s_1,\dots s_n) \defeq K(s_1) + \dots + K(s_n)$ for $s_i \in \EXPR$ if none of the above definitions apply.
    \end{enumerate}
}{}
\lemma{}{
...
%\section{A parsing algorithm for first order logic} Not in this years lecture
...
\defin{Deduction}{A formal proof (decuction) of a fla $\varphi$ from a set of formulas $\Sigma$ is a finite sequence of formulas 
    $(\alpha_0,\alpha_1,\dots,\alpha_n)$ such that $\alpha_n = \varphi$ and for every $i\leq n$ $\alpha_i$ is either a logical axiom or $\alpha_i\in\Sigma$ or 
...
\defin{substitutable}{\addAbbrev{SUB}{substitutable}
    ...
    \begin{enumerate}
        ...
        \item  $t$ is SUB for $x$ in $\lnot \alpha$ iff  $t$ is SUB for $x$ in $\alpha$
        \item  $t$ is SUB for $x$ in $\alpha\to \beta$ iff $t$ is SUB for $x$ in $\alpha$ and $\beta$
        \item  $t$ is SUB for $x$ in $\forall y \alpha$ iff either
        \begin{itemize}
...
\bsp{}{Consider the formula
\[\lnot (\forall y (Px \to Py))\to (Px\to \forall y \lnot Py)\]
which is built up from $\varphi \equiv \lnot (\forall y (Px \to Py))$ and $\psi \equiv Px\to \forall y \lnot Py$. Furthermore,
$\varphi$ is built up from $\forall y (Px \to Py)$ and 
$\psi$ is built up from $Px$ and  $\forall y \lnot Py$. The latter beeing prime formulas.
}
...
\note{}{%TODO make def below
    The definition of tautological implication $\Gamma \models_{\text{taut}} \varphi$ from propositional logic can be translated to \graybf{tautological implication} in first order logic.\\
    Let $\Gamma$ be a set of first order formulas, $\varphi$ a first order formula and $\mathbb{A} = \{A_1, A_2,\dots\}$ our set of atoms. We aim to construct a propositional sentence $\beta$ that behaves in some sense similar to $\varphi$ and a propositional sentence $\alpha_\gamma$ for each $\gamma\in \Gamma$ in the same way.
    If we 
}
\defin{tautological implication}{
    Let $\Gamma$ be a set of first order formulas and $\varphi$ a first order formula. We say 
    $\Gamma$ tautologically implies $\varphi$, iff 
    TODO
}
...
\thm{}{$\Gamma\vdash \varphi$ iff $\Gamma \cup \Lambda \models_{\text{taut}}\varphi$}{
    \begin{itemize}
        ...
        \item Assume $\Gamma \cup \Lambda \models_{\text{taut}}\varphi$. Then by the compactness theorem for propositional logic there are $\gamma_1\dots, \gamma_n\in\Gamma$ and $\lambda_1,\dots \lambda_m\in\Lambda$ such that
            \[\gamma_1\to \gamma_2\to \dots \to \gamma_n \to \lambda_1\to \dots \to \lambda_m\to \varphi \]
            is a tautology (always grouped to the right) %TODO check grouping direction
            because $\Gamma \cup \{\alpha\}\models_{\text{taut}}\beta$ iff $\Gamma \models_{\text{taut}}(\alpha \to \beta)$
    \end{itemize}
}
...
\begin{enumerate}
    ...
    (\cite[Exercise 11 of 2.4][130]{EndertonHerbertB2001AMIt})
    \item $\vdash \forall x_1 \forall x_2 \forall y_1 \forall y_2 (x_1=y_1 \to (x_2=y_2 \to (Px_1x_2 \to Py_1y_2)))$, similarly for any $n$-ary predicate. (see \cite[p.128]{EndertonHerbertB2001AMIt})
    \item $\vdash \forall x_1 \forall x_2 \forall y_1 \forall y_2 (x_1=y_1 \to (x_2 = y_2 \to (fx_1x_2 = fy_1y_2)))$, similarly for $n$-ary formula symbol, (see \cite[p.122]{EndertonHerbertB2001AMIt})
\end{enumerate}
...
\end{itemize}
\section{Soundness of first order logic}
...
\thm{Soundness Theorem}{If $\Gamma \vdash \varphi$ then $\Gamma \models \varphi$}{
...
\note{}{\ref{FO:SAT_implies_CON} is equivalent to Soundness (see Exercises)}
    ...
    \begin{enumerate}
        ...
        \begin{claimproof}
            \begin{itemize}
                ...
                \[
                \begin{aligned}
                    \mathcal{A}\models \varphi^*\to \psi^* \: [s] 
                    &\iif \mathcal{A}\not\models \varphi^* \: [s] \text{ or }
                    \mathcal{A}\models \psi^* \: [s] \\
                    &\iif \mathcal{A}\models \lnot \varphi^* \: [s] \text{ or }
                    \mathcal{A}\models \psi^* \: [s] \\
                    & \iif \lnot \varphi \in \Delta \text{ or } 
                    \psi\in \Delta \\
                    &\iif (\varphi \to \psi)\in \Delta
                \end{aligned}\]
...
%Exam exam
%TODO what does that above mean?
\datenote{14.11.2024}
...
\subsection{Sizes of models / completness for uncountable languages}
...
\lemma{}{
    Let $\Gamma$ be a set of formulas such that all models are finite. Then there has to be an
    $m\in\NN$ such that $|\mathcal{A}|\leq m$ for every model 
    $\mathcal{A}\in\Mod\Gamma$}{  Suppose $\Gamma$ has models of arbitrarily large finite size. \\

...
\section{Löwenheim-Skolem-Theorem}
...
\prop{}{The theory of algebraic closed fields of characteristic $p$ ACF$_p$, where $p$ is either $0$ or prime is complete.}{Note that we have a
\begin{itemize}
...
\defin{Cell}{
    ...
    \begin{itemize}
        ...
        \item $(i_1,\dots i_k, 0)$-cell: $\Gamma f\subseteq R^{k+1}$, where $f$ is definable and continuous function $f:$TODO
...
\note{Cells have nice topological properties}{
    \begin{itemize}
        ...
        \item Each cells is cocally closed i.e. open in its closure.
...
\thm{Cell decomposition}{
    \begin{itemize}
        ...
        \item[$(\RomanNum{2} _m)$] Given a definalble function $f: A\to R$, $A\subseteq R^m$ there is a decomposition $\mathcal{D}$ of $R^m$ partitioning $A$ such that for every $B\in \mathcal{D}$ $f|_B:B\to R$ is continuous. 
    \end{itemize}
}{
    By induction on $m$.

...
But ZFC knows that there are uncountable sets (see Cantor's \ref{5:Thm:Cantor}).
...
\begin{itemize}
    \item $\mathcal{L} = \{0,1,+,\cdot,-\}$ $\mathcal{F}$ the class of fields then $\Th(\mathcal{F})$ is the set of sentences true in every field.
\end{itemize}
}
...
\bsp{}{
    \begin{itemize}
        ...
        \item The theory of fields of characteristic $0$ is axiomatizable. Let $\Psi$ be the finitely many axioms of fields. $\Psi\cup \{1+1\neq 0, 1+1+1\neq 0,\dots\}$ axiomatizes the fields of characteristic $0$.
...
\prop{Lefschetz Principle}{Let $\mathcal{C} = (\mathbb{C},0,1,+,\cdot,-)$ For a sentence in the language of $\mathcal{C}$
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item for all sufficiently large primes $p$ it is ACF$_p$ $\models \sigma$
        \item For infinitely many primes $p$ it is ACF$_p$ $models \sigma$ %could also state that:  exists $\mathcal{K}$ structure that logically implies $\sigma$.
    \end{enumerate}}{ 
    Sketch:
    \begin{itemize}[leftmargin=2cm]
        ...
        \item[(d)$\implies$(e)] If it is true for all sufficiently large primes that it holds for infinitely many.
        \item[(e)$\implies$(c)] If $\text{ACF}_0\vDash \sigma$ than $\text{ACF}_0\models \sigma$
    \end{itemize}
}
Example of the Lefschetz Principle:
\datenote{21.11.2024}
\prop{Ax-Grothendieck}{\footnote{Named by Alexander Grothendieck and James Burton Ax}
...
\prop{}{
    For every $x\in \mathcal{F}$ there is exactly one $r\in \RR$ such that $x\sim r$
}{
...
\defin{}{We will say that $F'(a) = b$, iff for all $dx\in \mathcal{I}$, $dx\neq 0$ we have 
\[\st\biggl(\frac{F(a+dx)-F(a)}{dx}\biggr) = b\]
}%TODO check pls
...
\note{}{
    \begin{itemize}
        ...
        $(x,y)\to x \cdot y$
        $x\to x^{-1}$ are continuous with resprect to intervall topology.
    \end{itemize}
}
...
\prop{}{Let $(M,<,+,\cdot,\dots)$ be an o-minimal structure, such that $(M,<,+,\cdot)$ is an ordered ring. Then $(M,<,+,\cdot)$ is already an RCF.}{
    \begin{itemize}
...
%TODO: what does that mean? are we misssing something here?
...
\section{Cell decomposition Theorem}

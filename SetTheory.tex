...
\thm{Transfinite Recursion}{
    Let $A$ be a class, $M$ be a class of all functions $f:\alpha\to X$ for $\alpha\in\On$ arb.
    $X$ a subset of $A$ and $H$ a class function in one variable defined on all of $M$ with values in $A$.
    Then there exists a unique class function $F$ defined on $\On$ such that $\forall \alpha F(\alpha) = H(F|_\alpha)$
}{
    define $F$ by 
...
\thm{Zermelo}{
    ...
    and get an $H$-inductive class function $F:\On\to X$ 
    which is injective
...
\thm{Zorn's Lemma}{
    ...
    \begin{enumerate*}[label = (\roman*)]
        \item $f$ is a function with $\dom f \in \On$
        \item $\imag f\in A$
        \item $y = \pi(\imag f)$
    \end{enumerate*}
    ...
    If $H$ is actually defined on all $H$-inductive functions, then there is an $H$-inductive class function $F:\On \to X$.
...
Or how to think of the natural numbers to be contained in $\On$

\defin{}{
    \begin{itemize}
        \item $0\defeq \varnothing$ (smallest ordinal)
        \item $\beta\in \On$ then its successor ordinal is $\beta+1 \defeq \beta\cup\{\beta\}$
        \item ($\beta$ is a successor ordinal if there is $\alpha\in \On$ such that $\beta = \alpha + 1$)
        \item $\beta\in \On$ is a limit ordinal if $\beta \neq 0$ and $\beta$ is not a successor ordinal
        \item $\beta\in \On$ is a natual number / finite ordinal, if $\beta = 0$ or for every $\alpha\leq \beta$ $\alpha$ is a successor ordinal or $\alpha = 0$
    \end{itemize}
}
\bsp{}{
...
\note{}{
    $(X,<_X), (Y,<_Y)$ well-ordered then we can also order their disjoint union 
    $(X\times \{0\})\cup (Y\times \{1\})$, $X\times Y$ can be well ordered as follows:

    $$(a,i)<(b,j) \iif \begin{cases}
        i=j=0 & a<_X b\\
        \lor \: i=j=1 & a<_Y b\\
        \lor \: i<j
    \end{cases}$$
    and we will call this the ordering on the sum of $X$ and $Y$.\\
    $(x_0,x_1)<(x_1,y_1)\iif y_0<_Y y_1 \lor (y_0 = y_1\land x_0<_X x_1)$ (reverse lexiographical ordering)
}%TODO define sum of ordinals
\defin{}{
    Let $\alpha,\beta\in\On$ then 
    \begin{enumerate}[label=(\roman*)]
        \item $\alpha+\beta$ is the unique $\gamma\in \On$ such that $\gamma$ is order-isomorphic to the 
        sum of $\alpha$ and $\beta$
        \item $\alpha\cdot\beta$ is the unique $\gamma\in\On$ such that $\gamma$ is order-isomorphic to the 
...
\note{}{
    \begin{itemize}
        \item The natural numbers form an initial segment in $\On$.
...
\defin{}{
    ...
    \begin{enumerate}
        ...
        \item $\alpha^\lambda = \sup_{\delta < \lambda}{\alpha^\delta}$ for a limit ordinal $\lambda$
    \end{enumerate}
}
\note{}{
    can also define exponentiation as given by the class function $EXP : \On \times \On \to \On$ defined by
    \[Exp(\alpha,\beta) = \gamma \iif\]
    There exists a function $f:\beta+1\to \On \text{s.th.} \forall \xi<\beta$
    \begin{itemize}
        \item $f(\beta) = \gamma$
...
\defin{Cardinal}{$\kappa\in\On$ is called a cardinal, if $\kappa = |\kappa|$}
\note{}{The class $Card$ of all cardinals is a proper class.
\begin{claimproof}
    Suppose $Card$ is a set. then $\sup Card = \gamma\in \On$
...
\note{}{
    $Card$ is cofinial in $\On$
    For every $\alpha\in \On$ there is $\gamma\in Card$ with $\alpha<\gamma$

    And $Card$ is a proper subclass of $\On$, well ordered by the same ordering $\in$ as in $\On$
...
\thm{}{
    ...
    \[(\alpha,\beta)\prec (\alpha',\beta')\iff \begin{cases}
        \max\{\alpha,\beta\}<\max\{\alpha',\beta'\}\\
        \max\{\alpha,\beta\} = \max\{\alpha',\beta'\}\text{ and } \alpha<\alpha'\\
    ...
    \begin{claimproof}
        ...
        Consider $(\xi\times\xi,\prec)$ then by our inductive assumption this will have to be isomorphic to some ordinal $\gamma\in\On$.
...
The cofiniality of $\beta\in\On$ (denoted by $cof(\beta)$)
...
\note{}{ $cof (\beta)$ is always a cardinal.
    Let $\beta\in \On$ and let $f:cof(\beta)\to \beta$ be cofinal.
...
\defin{}{
    Class function
    $V:\On \to \mathcal{U}$ by transfinite induction on ordinals 
    ...
    $V$ is also a class given by $V(x)\equiv \exists \alpha\in\On\: x\in V_\alpha$
}
...
\lemma{}{
    ...
    Note that $\rk : V\to \On$ is bounded on $x$.
    Else, $\{\rk y : y \in x\}$ is unbounded in $\On$ and it is a set (image of function $\rk$ of a set)
    Take $\bigcup\{\rk y : y\in x\}\in \On$ and it would be an ordinal bigger than any other set, TODO

    Suppose $\{\rk y : y\in x\}$ is bounded by $\beta$ then $\forall y \in x y\in V_\beta$
    so $x\in V_{\beta+1}$.
}
\lemma{}{
    For every ordinal $\alpha\in \On$ we have $V(\alpha)$ and $\rk (\alpha) = \alpha+1$.
}{Exercise}
...
\thm{}{
    ...
    $V = \bigcup_{\alpha\in \On}V_\alpha$, $V_\beta \defeq \bigcup_{\alpha<\beta}{\mathcal{P}(V_\alpha)}$
...
\lemma{}{\label{lem:inacc1}
    ...
    Recall, if $\alpha\in \On$ then $\alpha\subseteq V_\alpha$. i.p.
    ...
    \begin{itemize}
        ...
        $\rk: a \to \On$ is not cofinal in $\kappa$ because $cof \kappa = \kappa > |a|$
...
\thm{}{
    ...
    \begin{claimproof}
        ...
        {equation:ordinal} holds. and $\alpha\in \On^{V_\kappa}$
        If $\alpha\in \On^{V_\kappa}$ then $\alpha\in V_\kappa$ and $\alpha$ is transitive and totally 
        ordered by $\in$.
    ...
    \begin{claimproof}
        ...
        Claim 1, $\alpha\in \On^{V_\kappa}$ and $f\subseteq V_\kappa$ (the graph $f\subseteq \lambda\times 

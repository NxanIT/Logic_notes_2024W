...
\defin{Axiom of Choice (AC)}{
    For every set $X$ and $A\subseteq \mathcal{P}(X)$ that consists of pairwise disjoint, non-empty subsets of $X$ there is a set $T\subseteq X$ such that $\forall a\in A \:\# a\cap T = 1$. $T$ is also called a transversal.
}
\defin{(AC')}{
    (Existence of choice function.) 
    For every set $X$ there exists a function $\pi : \mathcal{P}\backslash \{\varnothing\}\to X$ such that for every non-empty subset $a\subseteq X$ $\pi(a)\in a$
}
\defin{(AC'')}{
    Let $(X_i)_{i\in I}$ be an indexed family of TODO non-empty sets then $\prod_{i\in I}{X_i}\neq \varnothing$
}
...
\note{}{
    $(X,<_X), (Y,<_Y)$ well-ordered then we can also order their disjoint union $(X\times \{0\})\cup (Y\times \{1\})$, $X\times Y$ can be well ordered as follows:
...
\defin{}{
    ...
    $\alpha+\beta$ is the unique $\gamma\in \On$ such that $\gamma$ is order-isomorphic to the sum of $\alpha$ and $\beta$

    $\alpha\cdot\beta$ is the unique $\gamma\in\On$ such that $\gamma$ is order-isomorphic to product of $\alpha$ and $\beta$
}
Properties:
\lemma{}{
    \begin{enumerate}
...
\defin{Axiom of infinity}{
    ...
    i.e. ordinal that is not a natural number.
}
\note{}{
    \begin{itemize}
